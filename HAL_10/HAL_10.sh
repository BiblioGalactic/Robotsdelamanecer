...
# === Script de ejecución para el perfil HAL_10 ===
...
Descripción:
  Ejecuta inferencia con llama-cli usando el perfil HAL_10.
...
Archivos:
  HAL_10.txt      Prompt de personalidad (requerido)
...
PROMPT_FILE="${SCRIPT_DIR}/HAL_10.txt"
...
LOG_FILE="${SCRIPT_DIR}/run_HAL_10_$(date +%Y%m%d_%H%M%S).log"
...
rotate_old_logs() {
  ...
  local log_count
  log_count=$(find "$log_dir" -maxdepth 1 -name "run_HAL_10_*.log" -type f 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$log_count" -gt "$max_logs" ]]; then
    find "$log_dir" -maxdepth 1 -name "run_HAL_10_*.log" -type f -print0 \
      | xargs -0 ls -t \
      | tail -n +"$((max_logs + 1))" \
...
ejecutar() {
  ...
  local output_file="$SCRIPT_DIR/destino/conversaciones_HAL_10.txt"
