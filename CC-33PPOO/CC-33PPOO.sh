...
# === Script de ejecución para el perfil CC-33PPOO ===
...
show_help() {
    cat <<'HELP'
Uso: ./CC-33PPOO.sh [--help]

Descripción:
  Ejecuta inferencia con llama-cli usando el perfil CC-33PPOO.
...
Archivos:
  CC-33PPOO.txt   Prompt de personalidad (requerido)
...
PROMPT_FILE="${SCRIPT_DIR}/CC-33PPOO.txt"
...
LOG_FILE="${SCRIPT_DIR}/run_CC-33PPOO_$(date +%Y%m%d_%H%M%S).log"
...
rotate_old_logs() {
  ...
  local log_count
  log_count=$(find "$SCRIPT_DIR" -maxdepth 1 -name "run_CC-33PPOO_*.log" -type f 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$log_count" -gt "$max_logs" ]]; then
    find "$SCRIPT_DIR" -maxdepth 1 -name "run_CC-33PPOO_*.log" -type f -print0 \
      | xargs -0 ls -t \
      | tail -n "+$((max_logs + 1))" \
...
ejecutar() {
  ...
  local output_file="$SCRIPT_DIR/destino/conversaciones_CC-33PPOO.txt"
