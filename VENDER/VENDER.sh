...
Descripción:
  Ejecuta inferencia con llama-cli usando el perfil Bender/VENDER.
...
Archivos:
  VENDER.txt      Prompt de personalidad (requerido)
...
rotate_old_logs() {
  ...
  local log_count
  log_count=$(find "$SCRIPT_DIR" -maxdepth 1 -name "run_VENDER_*.log" -type f 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$log_count" -gt "$max_logs" ]]; then
    find "$SCRIPT_DIR" -maxdepth 1 -name "run_VENDER_*.log" -type f -print0 \
      | xargs -0 ls -t \
      | tail -n "+$((max_logs + 1))" \
